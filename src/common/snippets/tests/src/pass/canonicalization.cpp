...
TEST_P(CanonicalizationTests, Add) {
    ...
    PartialShape canonical_output_shape = subgraph->canonicalize(output_blocked_shapes, input_blocked_shapes);
    ASSERT_DIMS_EQ(canonical_output_shape.to_shape(), expected_output_shape);
}
